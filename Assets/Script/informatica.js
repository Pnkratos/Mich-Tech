...
function habilitarRecursos() {
  var lugarTitulo = document.getElementById("lugtituinfo");
  lugarTitulo.innerHTML = `<a href='smartphones.html' style='text-align:center;'><h1 id='titsec'>Informática</h1><br><br><br></a>`;

  var lugSmartphones = document.getElementById("Informatica");
  function renderizarProduto() {
    for (const dados of utilidades) {
      if (dados.cate == "informatica") {
  ...
  const celulares = utilidades.filter(
    (produto) => produto.cate === "informatica"
  );
...
const marcasDeCelulares = [
  ...
  const cateFiltrada = utilidades.filter((c)=>c.cate == 'informatica')
...
document.getElementById("adsa").addEventListener('click', function(){
  ...
  document.getElementById("Informatica").innerHTML=''
...
// adiciona funcionalidade aos botões
  ...
  function testederecepcao(item){
    ...
    var lugProdutosFiltrados = document.getElementById('Informatica')
...
function redesenharProdutos(idpro) {
  ...
  const lugarTag = document.getElementById(
    `produtoNoaCarrinho${product.id}`
  ); /*Mudei */
...
function filtrarProdutos() {
  ...
  for (const produtos of utilidades) {
    var btnAbrirPesquisa = document.getElementById(
      `btnPesquisaProduto${produtos.id}`
    );
    btnAbrirPesquisa.addEventListener("click", () =>
      aparecerProdutoIndividual(produtos.id)
    );
  }
}
filtrarProdutos();
...
function habilitarProdutoIndividual(id) {
  const produtos = utilidades.find((p) => p.id == id);
  ...
  const model2 = `    <div class='totalProdutoIndividual'>
    <button class='btnFecharProdutoIndividual' id='btnFecharProdutoIndividual${
      produtos.id
    }'>    <img src="./Assets/botoes/close_FILL1_wght400_GRAD0_opsz24B.svg" alt="" style='scale:1.2;'></button>

    <div class='left-side'>
    <span style='color:orange; position:absolute; top:0; margin-top:3%;'><a style='color:orange;' href='./index.html'>Início</a> / <a
    style='color:orange;' href='./${produtos.cate}.html'>${
    produtos.cate
  }</a></span>
        ...
        <div class="caracteristicasProdutoIndividual">
            ...
            <span class="semjurosProdutoIndividual">
                Em até <span class='txtsemjuros'> 10 X de R$${(
                  produtos.preco / 10
                ).toFixed(2)}</span>
            </span>
  ...
  var btnimgFecharProduto = document.getElementById(
    `btnFecharProdutoIndividual${produtos.id}`
  );
  btnimgFecharProduto.addEventListener("click", () =>
    aparecerProdutoIndividual(produtos.id)
  );

  var btnADDCARProdutoIndividual = document.getElementById(
    `2pro${produtos.id}`
  );
  btnADDCARProdutoIndividual.addEventListener("click", () =>
    renderizarProdutoCarrinho(produtos.id)
  );
