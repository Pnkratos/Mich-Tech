...
for(const adbtn of utilidades){
  if (adbtn.cate == "smartphones") {
    document.getElementById(`pro${adbtn.id}`).addEventListener('click',()=> renderizarProdutoCarrinho(adbtn.id))
  }
}
...
/*Renderiza os produtos*/
  ...
  const celulares = utilidades.filter(
    (produto) => produto.cate === "smartphones"
  );
...
const marcasDeCelulares = [
  ...
  const cateFiltrada = utilidades.filter((c)=>c.cate == 'smartphones')
...
document.getElementById("adsa").addEventListener('click', function(){
  ...
  document.getElementById("Smartphones1").innerHTML=''
...
// adiciona funcionalidade aos botões
  ...
  function testederecepcao(item){
    ...
    var lugProdutosFiltrados = document.getElementById('Smartphones1')
