...
function habilitarRecursos() {
  ...
  numeroCartaoProduto.addEventListener("input", () =>
    formatarCartao(numeroCartaoProduto)
  );
}
habilitarRecursos();
...
function renderizarProdutoFinal() {
  ...
  for (const idPro in idsProdutosCarrinho) {
    ...
    var divEvol = document.createElement("div");
    divEvol.setAttribute("class", "produtoCheckoutIndividual");
    divEvol.setAttribute("id", `produtoCheckoutIndividual${product.id}`);
    lugarFinal.appendChild(divEvol);
    var lugProdutos = document.getElementById(
      `produtoCheckoutIndividual${product.id}`
    );
...
var btncomprar = document.getElementById("btnPagar");
var lugarci = document.getElementById("contentPrincipal");

btncomprar.addEventListener("click", function () {
  lugarci.innerHTML = "";
  lugarci.innerHTML =
    "<h1>Obrigado por comprar conosco!</h1><br><br><button id='btnVoltarAComprar' class='btnPagar ' style='width:200px;'>Voltar a comprar</button>";
    document.getElementById("btnVoltarAComprar").addEventListener('click',()=> redirecionarIndex())
});
...
function calcularTotal(PrecoFrete) {
  ...
  lugTotalPreco.innerHTML = `R$ ${valorLiquido.toFixed(2).replace(".", ",")}`;
}

function removerProduto(idPro) {
  delete idsProdutosCarrinho[idPro];
  mostrarProdutosAtualizados(idPro);
...
function mostrarProdutosAtualizados(idPro) {
  document.getElementById(`produtoCheckoutIndividual${idPro}`).style.display =
    "none";
}
...
function imgVazioCarrinho() {
  ...
  if (contadorID >= 1) {
    ...
    lugarFinal.innerHTML = `<p style='color:black; display:flex; flex-direction:column; align-items:center; justify-content:space-around; gap:50PX; font-size:20pt;' id='txtVazio'> <img src='./Assets/botoes/shopping_bag_FILL1_wght500_GRAD0_opsz24.svg' style='height:150px;'>Ops! Você não tem produtos no carrinho <button class='btnVoltarInicial' id='btnVoltInicial'>Página Inicial</button></p>`;
    console.log("ta vazio");
    document.getElementById("btnVoltInicial").addEventListener("click", function(){
